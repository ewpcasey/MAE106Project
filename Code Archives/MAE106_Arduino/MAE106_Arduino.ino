...
double desiredPosition = 150;		// desired position (in degrees) we want the motor to go to
...
//  potAngle = map(analogRead(analogInPin), 0, 1023, 30, 330); 
}
...
void loop() {
  ...
  potAngle = map(sensorValue, 0, 1023, 30, 330); 
  ...
  if (count >= 500) {		// every 500 ms, change the desired position of the motor (simulates a square wave input)
	count = 0;
	if (desiredPosition == 150) {
		desiredPosition = 180;
	}
        else if (desiredPosition == 180){
		desiredPosition = 210;
	}
        else{
          desiredPosition = 150;
